...
// SIMULACIÓN DE CLIENTE REGISTRADO EN DOJA
...
function App() {
  ...
  // Pedidos simulados para la administración
  ...
  const finalizarPedido = () => {
    ...
    const nuevoPedidoId = `DOJA-${Math.floor(1000 + Math.random() * 9000)}`;
    const nuevo = {
      id: nuevoPedidoId,
    ...
    // Formateo del mensaje estructurado para WhatsApp
    const listaTexto = nuevo.items.map(it => `• ${it.quantity}x ${it.name}`).join('\n');
    const mensajeWA = `¡NUEVO PEDIDO ENVIADO! 🌸\n\n` +
                      `🆔 ID: ${nuevo.id}\n` +
                      `👤 CLIENTE: ${nuevo.clientName}\n` +
                      `📞 TELÉFONO: ${nuevo.phone}\n` +
                      `📍 DIRECCIÓN: ${nuevo.address}\n` +
                      `⏰ RANGO HORARIO: ${nuevo.schedule}\n` +
                      `📝 NOTAS: "${nuevo.instructions}"\n\n` +
                      `🛍️ PRODUCTOS:\n${listaTexto}\n\n` +
  ...
  return (
    <div className="min-h-screen font-sans bg-[#FFF6F7] text-[#5C1B26]">
      ...
      <div className="bg-[#5C1B26] text-[#FFE3E7] p-3 flex flex-col sm:flex-row justify-between items-center gap-2 text-xs font-medium sticky top-0 z-50 shadow-sm border-b border-[#7C3543]">
        <div>
          ...
          <span>Prueba el ecosistema alternando el rol aquí:</span>
        </div>
      ...
      {/* Alerta flotante */}
      {notificacion && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-[#E58B9D] text-white px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-2 border border-[#FFCAD2]">
      ...
      {/* HEADER PRINCIPAL */}
      <header className="bg-white/95 border-b border-[#FCD3D9] backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-serif font-black tracking-widest text-[#7C3543] cursor-pointer" onClick={() => setTabCliente('tienda')}>DOJA</h1>
          ...
          <div className="flex items-center gap-3">
            {role === 'customer' && (
              <button 
                onClick={() => { setTabCliente('carrito'); setFaseCompra('carrito'); }}
                className="relative p-2.5 bg-gradient-to-r from-[#E58B9D] to-[#FCAEB9] hover:from-[#C56B77] hover:to-[#E58B9D] text-white rounded-xl shadow-xs flex items-center gap-2 font-bold text-xs uppercase tracking-wider transition"
              >
        ...
        {role === 'customer' && (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-[#FFF0F2] via-[#FFF5F6] to-[#FFE3E7] border border-[#FCD3D9] p-6 sm:p-10 rounded-3xl relative overflow-hidden">
              <div className="relative z-10 max-w-xl">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-[#E58B9D] text-white mb-4">
                  <Sparkles className="w-3 h-3" /> COMPRA RÁPIDA DE COMÓDICOS
                </span>
                ...
                <p className="text-xs text-[#8C3A48] font-medium leading-relaxed mb-4">
                  Tu dirección de entrega, horarios y teléfono quedan guardados automáticamente en tu perfil fijo. ¡Olvidate de escribir dónde vivís en cada compra!
                </p>
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => setTabCliente('tienda')} className="px-4 py-2 bg-[#E58B9D] text-white rounded-xl text-xs font-bold hover:bg-[#C56B77] transition">Explorar Tienda</button>
                  <button onClick={() => setTabCliente('perfil')} className="px-4 py-2 bg-white text-[#7C3543] border border-[#FCD3D9] rounded-xl text-xs font-bold hover:bg-[#FFF0F2] transition">Ficha de Envío</button>
                </div>
              </div>
            ...
            <div className="flex gap-2 border-b border-[#FCD3D9] pb-1 overflow-x-auto">
              {['tienda', 'carrito', 'pedidos', 'perfil'].map((tabKey) => (
                <button
                  key={tabKey}
                  onClick={() => { setTabCliente(tabKey); setFaseCompra('carrito'); }}
                  className={`px-4 py-2 text-xs font-bold uppercase tracking-wider border-b-2 transition whitespace-nowrap ${
                    tabCliente === tabKey ? 'border-[#E58B9D] text-[#7C3543]' : 'border-transparent text-stone-400'
                  }`}
                >
                  {tabKey === 'tienda' ? '🎀 Catálogo' : tabKey === 'carrito' ? '🛒 Mi Carrito' : tabKey === 'pedidos' ? '📦 Mis Pedidos' : '📋 Mi Ficha Fija'}
                </button>
              ))}
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row gap-3">
                  <input
                    ...
                    className="flex-1 p-3 border border-[#FCD3D9] rounded-xl text-xs bg-white text-stone-800 focus:ring-1 focus:ring-[#E58B9D] focus:outline-none placeholder-stone-400 shadow-sm"
                  />
                    ...
                    {['TODOS', 'MAQUILLAJE', 'UÑAS PRESS ON', 'ACCESORIOS'].map(cat => (
                      <button
                        ...
                        className={`px-3 py-2 rounded-xl text-xs font-bold whitespace-nowrap border transition ${
                          filtroCategoria === cat ? 'bg-[#E58B9D] text-white border-transparent shadow-sm' : 'bg-white text-[#C56B77] border-[#FCD3D9] hover:bg-[#FFF0F2]'
                        }`}
                      >
                        {cat}
                      </button>
                  ...
                  {subcategoriasDisponibles.map(sub => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-xs font-semibold whitespace-nowrap transition ${
                        filtroSubcategoria === sub ? 'bg-[#FFF0F2] text-[#8C3A48] border border-[#FFD1D8]' : 'bg-white border text-stone-500 hover:bg-[#FFFDFD]'
                      }`}
                    >
                      {sub}
                    </button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {productosFiltrados.map(prod => (
                    <div key={prod.id} className="bg-white rounded-2xl border border-[#FCD3D9] overflow-hidden flex flex-col justify-between shadow-xs hover:shadow-sm transition duration-300">
              ...
              <div className="max-w-xl mx-auto bg-white border border-[#FCD3D9] rounded-3xl p-5 shadow-xs space-y-4 animate-fade-in">
                  ...
                  <div className="space-y-4">
                    ...
                    {carrito.length === 0 ? (
                      ...
                      <>
                        ...
                        <button onClick={() => setFaseCompra('pago')} className="w-full bg-[#E58B9D] hover:bg-[#C56B77] text-white py-3 rounded-xl text-xs font-bold shadow-xs transition">Avanzar al Pago Electrónico 💳</button>
                      </>
                    )}
                  </div>
                )}
                      ...
                      <div className="border-2 border-dashed border-[#FCD3D9] rounded-xl p-5 text-center bg-[#FFFBFB] space-y-3">
                        {comprobanteUrl ? (
                          <div className="space-y-2">
                            ...
                            <img src={comprobanteUrl} alt="Comprobante" className="max-h-24 mx-auto rounded border object-cover" />
                          ...
                          <div className="space-y-2">
                            ...
                            <button
                              ...
                              className="bg-[#7C3543] text-white text-[10px] px-3 py-1.5 rounded-lg font-bold hover:bg-[#5C2732] transition"
                            >
                ...
                {faseCompra === 'exito' && (
                  <div className="p-6 text-center space-y-4 animate-fade-in">
                    ...
                    <button onClick={() => { setFaseCompra('carrito'); setTabCliente('pedidos'); }} className="bg-[#E58B9D] text-white px-5 py-2 rounded-lg text-xs font-bold hover:bg-[#C56B77] transition">Ver Estado del Envío 📦</button>
                  </div>
                )}
              </div>
            )}
            ...
            {tabCliente === 'pedidos' && (
              <div className="max-w-xl mx-auto space-y-3 animate-fade-in">
                ...
                {pedidos.length === 0 ? (
                  <p className="text-xs text-stone-400 text-center py-6">No has realizado ninguna compra aún.</p>
                ) : (
                  pedidos.map(p => (
                    <div key={p.id} className="bg-white p-4 rounded-xl border border-[#FCD3D9] shadow-xs space-y-3">
                      <div className="flex justify-between items-center border-b pb-2 border-[#FFF0F2]">
                        <div>
                      ...
                      <div className="text-right font-black text-stone-900 text-xs pt-1 border-t border-[#FFF0F2]">
            ...
            {tabCliente === 'perfil' && (
              <div className="max-w-xl mx-auto bg-white p-5 rounded-2xl border border-[#FCD3D9] shadow-xs space-y-4 animate-fade-in">
                <div>
                  ...
                  <p className="text-xs text-stone-400">Guardá tus datos una sola vez. No te volveremos a pedir dirección, ubicación o rangos horarios en ninguna compra futura.</p>
                </div>
                <div className="space-y-3 text-xs">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block font-bold text-stone-600 mb-1">Nombre Completo:</label>
                      <input type="text" value={perfil.name} onChange={(e) => setPerfil({...perfil, name: e.target.value})} className="w-full p-2.5 border border-pink-100 rounded-lg bg-[#FAF8F7]" />
                    </div>
                    <div>
                      <label className="block font-bold text-stone-600 mb-1">WhatsApp de Contacto:</label>
                      <input type="text" value={perfil.phone} onChange={(e) => setPerfil({...perfil, phone: e.target.value})} className="w-full p-2.5 border border-pink-100 rounded-lg bg-[#FAF8F7]" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block font-bold text-stone-600 mb-1">Dirección Exacta:</label>
                      <input type="text" value={perfil.address} onChange={(e) => setPerfil({...perfil, address: e.target.value})} className="w-full p-2.5 border border-pink-100 rounded-lg bg-[#FAF8F7]" />
                    </div>
                    <div>
                      <label className="block font-bold text-stone-600 mb-1">Barrio / Localidad:</label>
                      <input type="text" value={perfil.neighborhood} onChange={(e) => setPerfil({...perfil, neighborhood: e.target.value})} className="w-full p-2.5 border border-pink-100 rounded-lg bg-[#FAF8F7]" />
                    </div>
                  </div>
                  <div>
                    <label className="block font-bold text-stone-600 mb-1">Franja Horaria de Disponibilidad:</label>
                    <select value={perfil.homeSchedule} onChange={(e) => setPerfil({...perfil, homeSchedule: e.target.value})} className="w-full p-2.5 border border-pink-100 rounded-lg bg-[#FAF8F7] bg-white">
                  ...
                  <div>
                    ...
                    <textarea rows="2" value={perfil.instructions} onChange={(e) => setPerfil({...perfil, instructions: e.target.value})} className="w-full p-2.5 border border-pink-100 rounded-lg bg-[#FAF8F7] focus:outline-none focus:ring-1 focus:ring-[#E58B9D]" placeholder="Ej: timbre roto, dejar con el encargado de PB, reja de color rosa..." />
                  </div>
                  <button onClick={() => mostrarNotificacion("✅ Ficha de entrega permanente actualizada.")} className="w-full bg-[#E58B9D] hover:bg-[#C56B77] text-white font-bold py-2.5 rounded-xl text-xs transition">Guardar Mis Datos de por Vida</button>
                </div>
              </div>
            )}
        ...
        {role === 'admin' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-[#7C3543] text-white p-5 rounded-2xl shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="flex bg-[#602D35] p-1 rounded-lg border border-[#7C3543] overflow-x-auto max-w-full">
                {['pedidos', 'clientes', 'catalogo'].map(adTab => (
                  <button 
                    key={adTab}
                    onClick={() => setTabAdmin(adTab)}
                    className={`px-3 py-1.5 rounded text-xs font-bold transition whitespace-nowrap ${tabAdmin === adTab ? 'bg-[#E58B9D] text-white' : 'text-stone-300'}`}
                  >
            ...
            {tabAdmin === 'pedidos' && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 space-y-3">
                  ...
                  {pedidos.length === 0 ? (
                    <p className="text-xs text-stone-400 text-center py-10 bg-white rounded-xl border border-[#FCD3D9]">No hay pedidos pendientes de entrega.</p>
                  ) : (
                    pedidos.map(p => (
                      <div 
                        key={p.id}
                ...
                <div className="bg-white p-5 rounded-2xl border border-[#FCD3D9] shadow-xs space-y-4 h-fit">
                  {pedidoSeleccionado ? (
                    <div className="space-y-4 text-xs">
                      <div className="border-b pb-2 border-pink-50">
                        <span className="text-[#8C4A43] font-bold tracking-widest text-[9px] uppercase block">Ficha para el Repartidor (Automática)</span>
                      ...
                      <div className="space-y-1 bg-stone-50 p-2.5 rounded-lg border border-stone-100 text-stone-600">
                        ...
                        <p className="pt-1 font-bold text-stone-900 border-t mt-1 border-stone-200">Total: ${pedidoSeleccionado.total.toLocaleString('es-AR')}</p>
                      </div>

                      <div>
                        <p className="font-bold text-stone-600 mb-1">Comprobante de Transferencia:</p>
                        <img src={pedidoSeleccionado.paymentReceipt} alt="Comprobante Pago" className="w-full rounded-lg border border-stone-100 max-h-36 object-cover shadow-inner" />
                      </div>

                      <div className="space-y-2 border-t pt-3 border-pink-50">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-white p-4 rounded-xl border border-[#FCD3D9] space-y-2 text-xs shadow-xs">
                    <p className="font-black text-[#8C3A48] text-sm border-b pb-1 border-pink-50">👤 {perfil.name}</p>
                    <p><strong>WhatsApp:</strong> {perfil.phone}</p>
                    <p><strong>Ubicación Permanente:</strong> {perfil.address}, {perfil.neighborhood}</p>
                    <p><strong>Franja horaria en casa:</strong> {perfil.homeSchedule}</p>
                    <p className="bg-[#FFF0F2] text-[#8C3A48] p-2.5 rounded-lg font-bold italic">" {perfil.instructions} "</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-2xl border border-[#FCD3D9] p-4 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {inventario.map(prod => (
                      <div key={prod.id} className="p-3 border border-[#FCD3D9] rounded-xl flex gap-3 items-center bg-[#FFFBFB] justify-between">
                        <div className="flex gap-3 items-center">
                          ...
                          <div className="space-y-0.5 text-xs">
                            ...
                            <p className="text-stone-400 text-[9px] uppercase tracking-wider">{prod.categoria} - SKU: {prod.sku}</p>
                          </div>
                        </div>
                      </div>
                    ))}
